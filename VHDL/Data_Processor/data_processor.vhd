...
entity data_processor is
	port(
		...
		dataResult: out STD_LOGIC_VECTOR(55 downto 0);	--Binary sequence, using 2's complement
...
architecture synth of data_processor is
	...
	signal buf, buf_next: memory;
	signal result, result_next: memory;
	...
	signal max_value, max_index: integer := 0;
	signal max_value_reg, max_index_reg: integer := 0;
...
begin
	...
	begin
		...
		result_next(3) <= result(3);
		result_next(4) <= result(4);
		result_next(5) <= result(5);
		result_next(6) <= result(6);
		...
		case current_state is
			...
			when processing =>
				...
				if counter = 4 then
					...
					if max_value < unsigned(buf_next(3)) then
						max_value_reg <= buf_next(3);
